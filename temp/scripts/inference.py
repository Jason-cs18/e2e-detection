...
class DataProcess(object):
    ...
    def postprocess(self, boxes_and_score_data, labels_data, out_file_name):
        self._draw_result_to_img(boxes_and_score_data, labels_data, out_file_name)
...
class PPLModel(object):
    ...
    def _create_runtime(self, model_file_name):
        ...
        if status != pplcommon.RC_SUCCESS:
            logging.error("OnnxRuntimeBuilder SetResources failed: " + pplcommon.GetRetCodeStr(status))
            sys.exit(-1)
    ...
    def run(self, engine_type, model_file_name, input_file):
        ...
        if status != pplcommon.RC_SUCCESS:
            ...
            sys.exit(-1)
        dets_data, labels_data = self._prepare_output()
        return dets_data, labels_data

def parsArgs():
    parser = argparse.ArgumentParser("runner of xxx onnx model.")
...
if __name__ == "__main__":
    ...
    # runmodel
    ppl_model = PPLModel()
    dets_data, labels_data = ppl_model.run('x86', model_file, input_data)

    # postprocess
    data_process.postprocess(dets_data, labels_data, result_file)
